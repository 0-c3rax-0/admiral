...
- **Carrier ships**: Carriers can haul other ships. If the API exposes grouped storage commands, load a ship with `storage(action="deposit", item_id=<ship_id>, target=self)` and unload it with `storage(action="withdraw", item_id=<ship_id>)`. If the API exposes `storage_deposit` / `storage_withdraw` aliases, those are equivalent.
